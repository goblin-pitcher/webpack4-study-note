...
const source = fs.readFileSync(path.resolve(__dirname, './template.js'),'utf8');
...
const genWebpackModuleTypes = (info) =>{
  const properties = Object.entries(info).reduce((arr, entry)=>{
    ...
    const value = t.arrowFunctionExpression(args.map(arg=>t.identifier(arg)), t.blockStatement([expression]))
...
module.exports = function (webpackModules) {
  const ast = parse(source);
  const objAst = genWebpackModuleTypes(webpackModules)
  traverse(ast, {
    VariableDeclarator(path) {
      if(path.node.id.name === '__webpack_modules__') {
